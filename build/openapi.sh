#!/bin/bash
#=====================================================================
# MarlinDT Network Intelligence (MNI) - OpenAPI YAML Pipeline
#
...
#yq -p yaml -o json api/mni.yaml > api/mni.json && \
yq "." api/mni.yaml > api/mni.json && \
git diff --quiet api/mni.json &>/dev/null || git add -f api/mni.json
RETVAL=$?
[[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
...
if [ ${RETVAL} -eq 0 ] ; then
 ...
 if [ $? -eq 0 ] ; then
   ...
   npx -y @redocly/cli@latest build-docs api/mni.yaml -o docs/api/mni.html &>/dev/null && \
   git diff --quiet docs/api/mni.html &>/dev/null || git add -f docs/api/mni.html
   RETVAL=$?
   [[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
 fi
fi
