...
if [[ -z "${SERVICE_USERNAME}" || -z "${SERVICE_KEY}" ]] ; then
  error "- Install MNI Api Server first or update ${CONFIG_DIRECTORY}/mni.ini accordingly"
  RETVAL=1
else
...
if [[ -z "${ADDRESS}" ]] ; then
  ...
  sed -i -e "s|APIGW_ADDRESS=.*|APIGW_ADDRESS=\"${ADDRESS}\"|" ${CONFIG_DIRECTORY}/mni.ini 
  RETVAL=$?
else
  RETVAL=0
fi
...
curl --fail --location --output /tmp/krakend-linux-amd64.tar.gz \
