...
cp -f ${CLI_PATH}/ni.yaml ${API_DIRECTORY}/ && chown ${USERNAME}:${GROUP} ${API_DIRECTORY}/ni.yaml && chmod 660 ${API_DIRECTORY}/ni.yaml && \
...
if [[ -z "${ADDRESS}" ]] ; then
  ...
  sed -i -e "s|APISERV_ADDRESS=.*|APISERV_ADDRESS=\"${ADDRESS}\"|" ${CONFIG_DIRECTORY}/ni.ini 
  RETVAL=$?
else
  RETVAL=0
fi
...
sed -i -e "s|/usr/local/ni/api/apiServer.mjs|${WORKING_DIRECTORY}/apiServer.mjs|" ${WORKING_DIRECTORY}/package.json && \
...
if [ "${API_DIRECTORY}/ni.yaml" ] ; then
 curl --silent -L --output yq "https://github.com/mikefarah/yq/releases/download/v4.45.1/yq_linux_amd64" && \
 chmod 700 ./yq && \
 ./yq --exit-status -i ".servers[].url = \"https://${APIGW_ADDRESS}:${APIGW_PORT}${APISERV_URL_PREFIX}${APISERV_URL_VERSION}\"" ${API_DIRECTORY}/ni.yaml &>/dev/null && \
 ./yq --exit-status "." -o=json ${API_DIRECTORY}/ni.yaml > ${API_DIRECTORY}/ni.json && \
 chown ${USERNAME}:${GROUP} ${API_DIRECTORY}/ni.json && chmod 660 ${API_DIRECTORY}/ni.json && \
 chown ${USERNAME}:${GROUP} ${API_DIRECTORY}/ni.yaml && chmod 660 ${API_DIRECTORY}/ni.yaml && \
 rm -f ./yq &>/dev/null
fi
RETVAL=$?
[[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
...
if [[ -z "${SERVICE_USERNAME}" ]] ; then
  ...
  sed -i -e "s|NI_SERVICE_USERNAME=.*|NI_SERVICE_USERNAME=\"${SERVICE_USERNAME}\"|" ${CONFIG_DIRECTORY}/ni.ini
  RETVAL=$?
  [[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
fi
...
if [[ -z "${SERVICE_KEY}" ]] ; then
  ...
  sed -i -e "s|NI_SERVICE_KEY=.*|NI_SERVICE_KEY=\"${SERVICE_KEY}\"|" ${CONFIG_DIRECTORY}/ni.ini
  RETVAL=$?
  [[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
fi
...
if [[ -z "${ENCRYPTION_KEY}" ]] ; then
  ...
  sed -i -e "s|APISERV_ENCRYPTION_KEY=.*|APISERV_ENCRYPTION_KEY=\"${ENCRYPTION_KEY}\"|" ${CONFIG_DIRECTORY}/ni.ini
  RETVAL=$?
  [[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
fi
...
if [[ -z "${ENCRYPTION_IV}" ]] ; then
  ...
  sed -i -e "s|APISERV_ENCRYPTION_IV=.*|APISERV_ENCRYPTION_IV=\"${ENCRYPTION_IV}\"|" ${CONFIG_DIRECTORY}/ni.ini
  RETVAL=$?
  [[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
fi
