...
CREATE TYPE cableState AS ENUM ('free','used','reserved','faulty','predicted');
...
CREATE TYPE ductSizeCategory AS ENUM ('duct','subduct');
CREATE TYPE ductState AS ENUM ('free','used','reserved','faulty','predicted');
...
CREATE TYPE hvacMode AS ENUM ('cooling','heating','ventilation','dependant');
...
CREATE TYPE portState AS ENUM ('free','used','reserved','faulty','predicted');
...
CREATE TYPE resourceType AS ENUM ('cable','duct','ne','pole','rack','service','site','trench');
CREATE TYPE resourceStateType AS ENUM ('create','delete','update','undelete','read');
...
CREATE TYPE slotState AS ENUM ('free','used','reserved','faulty','predicted');
...
CREATE TYPE trenchState AS ENUM ('free','used','reserved','faulty','predicted');
...
CREATE SEQUENCE IF NOT EXISTS seq_cableFiberState;
...
CREATE SEQUENCE IF NOT EXISTS seq_replicationQueue;
...
CREATE SEQUENCE IF NOT EXISTS seq_siteHvac;
...
CREATE TABLE IF NOT EXISTS predictQueue (
    qId INTEGER NOT NULL DEFAULT nextval('seq_predictQueue'),
    point TIMESTAMP NOT NULL DEFAULT now()::timestamp,
    delete BOOLEAN NOT NULL DEFAULT false,
    resource resourceType NOT NULL,
    id VARCHAR NOT NULL,
    state resourceStateType NOT NULL,
    PRIMARY KEY (resource,id)
);

CREATE TABLE IF NOT EXISTS replicationQueue (
    qId INTEGER NOT NULL DEFAULT nextval('seq_replicationQueue'),
    point TIMESTAMP NOT NULL DEFAULT now()::timestamp,
    delete BOOLEAN NOT NULL DEFAULT false,
    resource resourceType NOT NULL,
    id VARCHAR NOT NULL,
...
CREATE TABLE IF NOT EXISTS adminKafka (
    id VARCHAR NOT NULL DEFAULT uuid() PRIMARY KEY,
    name VARCHAR NOT NULL,
    clientId VARCHAR NOT NULL DEFAULT 'NI',
    host VARCHAR NOT NULL,
...
CREATE TABLE IF NOT EXISTS worldGeo (
    country countryCode NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS _trenchCoordinate (
    ...
    Z DECIMAL(18,2) NOT NULL DEFAULT 0 CHECK (Z >= -1230000 AND Z <= 10000000),
    M VARCHAR,
    geoPoint POINT_2D,
    FOREIGN KEY (trenchId) REFERENCES trench (id)
);
...
CREATE TABLE IF NOT EXISTS _siteHvac (
    tsId INTEGER NOT NULL DEFAULT nextval('seq_siteHvac') PRIMARY KEY,
    point TIMESTAMP NOT NULL DEFAULT now()::timestamp,
    siteId VARCHAR NOT NULL,
    siteTsId INTEGER NOT NULL,
    source source NOT NULL DEFAULT 'historical',
    probability DECIMAL(3,2) NOT NULL DEFAULT 1 CHECK (probability >= 0 AND probability <= 1),
    occupied BOOLEAN NOT NULL DEFAULT false,
    unit temperatureUnit DEFAULT 'celsius',
    operation hvacMode NOT NULL DEFAULT 'dependant',
    internalTemperature DECIMAL(5,2) DEFAULT 0.0,
    setPointTemperature DECIMAL(5,2) DEFAULT 0.0,
    externalTemperature DECIMAL(5,2) DEFAULT 0.0,
    internalHumidity DECIMAL(5,2) DEFAULT 0.0 CHECK (internalHumidity >= 0 AND internalHumidity <= 100),    
    externalHumidity DECIMAL(5,2) DEFAULT 0.0 CHECK (externalHumidity >= 0 AND externalHumidity <= 100),    
    stpEnergyEfficiency DECIMAL(5,2) DEFAULT 0.0,
    stpThermalOptimumEnergy DECIMAL(5,2) DEFAULT 0.0,
    stpThermalMinimumComfort DECIMAL(5,2) DEFAULT 0.0,
    stpThermalRecommendedComfort DECIMAL(5,2) DEFAULT 0.0,
    FOREIGN KEY (siteId) REFERENCES site (id),
    FOREIGN KEY (siteTsId) REFERENCES _site (tsId)
);
-- STP = Set-Point Temperature
...
CREATE TABLE IF NOT EXISTS _cableCoax (
    ...
    source source NOT NULL DEFAULT 'historical',
    cableId VARCHAR NOT NULL,
    frequencyLow DECIMAL(10,2) NOT NULL DEFAULT 0.1 CHECK (frequencyLow >= 0.1 AND frequencyLow <= 128000),
    frequencyHigh DECIMAL(10,2) NOT NULL DEFAULT 64738 CHECK (frequencyHigh >= 0.1 AND frequencyHigh <= 128000),
    channels INTEGER DEFAULT 1 CHECK (channels >= 1 AND channels <= 1024),
    width DECIMAL(10,2) DEFAULT 1 CHECK (width >= 0.1 AND width <= 128000),
...
CREATE TABLE IF NOT EXISTS _cableFiberState (
    tsId INTEGER NOT NULL DEFAULT nextval('seq_cableFiberState') PRIMARY KEY,
    point TIMESTAMP NOT NULL DEFAULT now()::timestamp,
    source source NOT NULL DEFAULT 'historical',
    fiberTsId INTEGER NOT NULL,
    cableId VARCHAR NOT NULL,
    ribbon INTEGER NOT NULL DEFAULT 0 CHECK (ribbon >= 0 AND ribbon <= 36),
    strand INTEGER NOT NULL CHECK (strand >= 1 AND strand <= 48),
    state cableState NOT NULL DEFAULT 'free',
    FOREIGN KEY (cableId) REFERENCES cable (id)
);
...
BEGIN TRANSACTION;
INSERT INTO adminData (id,historicalDuration,historicalUnit,predictedDuration,predictedUnit) VALUES (uuid(),1,'year',6,'month');
COMMIT;

BEGIN TRANSACTION;
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('5c8d258b-94d7-4826-afe0-bfb9a4237332','Trench Utilization 75%','jobAlertTrenchUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('d461d060-f19c-4c4a-956a-07a0997dd9b7','Trench Utilization 90%','jobAlertTrenchUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('447441f9-8360-4dcb-8d30-440ccf113f64','Trench Utilization 100%','jobAlertTrenchUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('6aaeef30-21aa-4bc1-adfd-97013d345816','Duct Utilization 75%','jobAlertDuctUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('0749db4d-bccb-4ecc-b070-dc9f9b115737','Duct Utilization 90%','jobAlertDuctUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('00f3868c-3304-4783-bb7a-904b093339b7','Duct Utilization 100%','jobAlertDuctUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('20e5c10c-f6c5-4d4c-a91d-d7d338207b8f','Cable Utilization 75%','jobAlertCableUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('24a49d2f-eb13-49ab-975c-fbbf678b924f','Cable Utilization 90%','jobAlertCableUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('4e7387e8-f5e8-498f-9593-a2495dd0e630','Cable Utilization 100%','jobAlertCableUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('c45b5854-0322-48c5-95e9-5c468c657bc8','Pole Utilization 75%','jobAlertPoleUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('cc1fab4f-8f79-4e3d-b24d-8829e0fa1dbe','Pole Utilization 90%','jobAlertPoleUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('7a5fb877-9fd0-4475-891a-7fcd7729bdac','Pole Utilization 100%','jobAlertPoleUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('ac67b9b4-35d8-4c4b-94ed-3321aaf1a300','Rack Utilization 75%','jobAlertRackUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('2555a838-2865-47d2-8f49-e191a955ff66','Rack Utilization 90%','jobAlertRackUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('83075e1e-ce6b-4718-9490-cffb54f8d956','Rack Utilization 100%','jobAlertRackUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('db97ee07-84eb-4de9-a7b8-7c6bc2d22500','NE Port Utilization 75%','jobAlertNePortUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('e566960f-12c4-4ce6-bb29-f94d18d5a97c','NE Port Utilization 90%','jobAlertNePortUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('7ec78138-0ce7-4344-96c5-0466b85b270e','NE Port Utilization 100%','jobAlertNePortUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('4626b921-baf4-4457-a6e4-661e62ecdba1','Transmission Utilization 75%','jobAlertTransmissionUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('596040f5-485e-47f5-a86f-578abee86c78','Transmission Utilization 90%','jobAlertTransmissionUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('1a2701e3-6540-47c4-97ed-c24782089d61','Transmission Utilization 100%','jobAlertTransmissionUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('87659ffc-a1a1-45b7-a704-84ef9cf4dd06','Service Bandwidth Utilization 75%','jobAlertServiceBandwidth(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('8626dcca-1a50-4ded-90c3-c918ad5af222','Service Bandwidth Utilization 90%','jobAlertServiceBandwidth(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('582d8c83-b5a9-47bb-8b34-cf1c8e610457','Service Bandwidth Utilization 100%','jobAlertServiceBandwidth(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('eafa41af-4007-44f1-88ee-870e8d60fe0c','NE Error Rates 75%','jobAlertNeErrorRates(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('c7dd4389-aa64-4f0a-83a9-f3c4988e8699','NE Error Rates 90%','jobAlertNeErrorRates(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('9108d9c0-1001-4c6f-b6f7-9d6e4cae0087','NE Error Rates 100%','jobAlertNeErrorRates(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('2e4866c7-14e4-4771-b3c5-cdd23e1ca8c5','NE Classifiers Utilization 75%','jobAlertNeClassifierUtil(75);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('7aa3a90d-10c1-40b4-8598-662083d6c462','NE Classifiers Utilization 90%','jobAlertNeClassifierUtil(90);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('e62df531-25e6-4b09-9a6d-9cf44b3eb77f','NE Classifiers Utilization 100%','jobAlertNeClassifierUtil(100);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('d863bc95-6e1b-4c2c-bea7-b6d55b808e85','NE CVE Scan','jobAlertNeCveScan();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('b7c0dbac-6871-4c92-a3f8-dfa925553319','Work Planning Efficiency 110%','jobAlertWpEfficiency(110);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('96ea3707-4d1f-4c7c-92c3-fa4e299135a0','Work Planning Efficiency 130%','jobAlertWpEfficiency(130);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('942c2393-6990-43bc-9971-4d7d1afa2ac1','Work Planning Efficiency 150%','jobAlertWpEfficiency(150);','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('3ef7ae9e-44a6-447c-9763-b6886748b5d4','Data Quality - geometry alignment','jobAlertDqGeometryAlignment();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('c7284f1f-6b73-4eec-ab27-49337043679c','Data Quality - missing connectedTo','jobAlertDqMissingConnectedTo();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('4f726282-3328-421d-bd5c-2d2ba5f23f95','Data Quality - trench','jobAlertDqTrench();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('e2a65c2b-7b88-4500-bdd7-3d648b42aa94','Data Quality - cable','jobAlertDqCable();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('ed0cf860-4a8f-4b05-aa70-b4071fca9a40','Data Quality - duct','jobAlertDqDuct();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('e2a4a205-29fa-417a-a6d0-b3c39ac7877e','Data Quality - pole','jobAlertDqPole();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('52ae75e2-036d-4a99-813f-de6ba328fce9','Data Quality - NE','jobAlertDqNe();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('126a7d5a-77af-4e99-9b6e-31b1945d5365','Data Quality - service','jobAlertDqService();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('c54f88d9-d355-46e0-8fb2-9fa6ccdc4083','Data Quality - site','jobAlertDqSite();','0 0 * * *',true);
INSERT INTO alert (id,description,function,cronTime,enabled) VALUES ('b034d9e8-da77-4b8c-8f44-d6eb6b6076a5','Data Quality - offNet Postal Addresses','jobAlertDqOffNetPAF();','0 0 * * *',true);
COMMIT;
...
BEGIN TRANSACTION;
INSERT INTO costCable (technology, unit, costPerUnit) VALUES ('coax','m',5);
INSERT INTO costCable (technology, unit, costPerUnit) VALUES ('copper','m',15);
INSERT INTO costCable (technology, unit, costPerUnit) VALUES ('ethernet','m',2);
INSERT INTO costCable (technology, unit, costPerUnit) VALUES ('singleFiber','m',35);
INSERT INTO costCable (technology, unit, costPerUnit) VALUES ('multiFiber','m',75);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',1,'m',50);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',10,'m',100);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',12,'m',120);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',14,'m',140);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',19,'m',190);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',2,'m',200);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',24,'m',240);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',3,'m',300);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',48,'m',4800);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',5,'m',500);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',6,'m',600);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',7,'m',700);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('duct',8,'m',800);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',1,'m',50);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',10,'m',100);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',12,'m',120);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',14,'m',140);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',19,'m',190);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',2,'m',200);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',24,'m',240);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',3,'m',300);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',48,'m',4800);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',5,'m',500);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',6,'m',600);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',7,'m',700);
INSERT INTO costDuct (category, configuration, unit, costPerUnit) VALUES ('subduct',8,'m',800);
...
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('backhaul','commercial',25);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('backhaul','highways',30);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('backhaul','rural',15);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('backhaul','unclassified',40);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('backhaul','urban',20);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('link','commercial',25);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('link','highways',30);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('link','rural',15);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('link','unclassified',40);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('link','urban',20);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('service/drop','commercial',25);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('service/drop','highways',30);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('service/drop','residential',15);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('service/drop','rural',15);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('service/drop','unclassified',40);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('service/drop','urban',13.50);
INSERT INTO costPole (purpose, classifier, costPerUnit) VALUES ('unclassified','unclassified',50);
INSERT INTO costRack (slots,costPerUnit) VALUES (1,1);
INSERT INTO costRack (slots,costPerUnit) VALUES (2,2);
INSERT INTO costRack (slots,costPerUnit) VALUES (3,3);
INSERT INTO costRack (slots,costPerUnit) VALUES (4,4);
INSERT INTO costRack (slots,costPerUnit) VALUES (5,5);
INSERT INTO costRack (slots,costPerUnit) VALUES (6,6);
INSERT INTO costRack (slots,costPerUnit) VALUES (7,7);
INSERT INTO costRack (slots,costPerUnit) VALUES (8,8);
INSERT INTO costRack (slots,costPerUnit) VALUES (9,9);
INSERT INTO costRack (slots,costPerUnit) VALUES (10,10);
INSERT INTO costRack (slots,costPerUnit) VALUES (11,11);
INSERT INTO costRack (slots,costPerUnit) VALUES (12,120);
INSERT INTO costRack (slots,costPerUnit) VALUES (13,130);
INSERT INTO costRack (slots,costPerUnit) VALUES (14,140);
INSERT INTO costRack (slots,costPerUnit) VALUES (15,150);
INSERT INTO costRack (slots,costPerUnit) VALUES (16,160);
INSERT INTO costRack (slots,costPerUnit) VALUES (17,170);
INSERT INTO costRack (slots,costPerUnit) VALUES (18,180);
INSERT INTO costRack (slots,costPerUnit) VALUES (19,190);
INSERT INTO costRack (slots,costPerUnit) VALUES (20,200);
INSERT INTO costRack (slots,costPerUnit) VALUES (21,210);
INSERT INTO costRack (slots,costPerUnit) VALUES (22,220);
INSERT INTO costRack (slots,costPerUnit) VALUES (23,230);
INSERT INTO costRack (slots,costPerUnit) VALUES (24,240);
INSERT INTO costRack (slots,costPerUnit) VALUES (25,250);
INSERT INTO costRack (slots,costPerUnit) VALUES (26,260);
INSERT INTO costRack (slots,costPerUnit) VALUES (27,270);
INSERT INTO costRack (slots,costPerUnit) VALUES (28,280);
INSERT INTO costRack (slots,costPerUnit) VALUES (29,290);
INSERT INTO costRack (slots,costPerUnit) VALUES (30,300);
INSERT INTO costRack (slots,costPerUnit) VALUES (31,310);
INSERT INTO costRack (slots,costPerUnit) VALUES (32,320);
INSERT INTO costRack (slots,costPerUnit) VALUES (33,330);
INSERT INTO costRack (slots,costPerUnit) VALUES (34,340);
INSERT INTO costRack (slots,costPerUnit) VALUES (35,350);
INSERT INTO costRack (slots,costPerUnit) VALUES (36,360);
INSERT INTO costRack (slots,costPerUnit) VALUES (37,370);
INSERT INTO costRack (slots,costPerUnit) VALUES (38,380);
INSERT INTO costRack (slots,costPerUnit) VALUES (39,390);
INSERT INTO costRack (slots,costPerUnit) VALUES (40,400);
INSERT INTO costRack (slots,costPerUnit) VALUES (41,410);
INSERT INTO costRack (slots,costPerUnit) VALUES (42,420);
INSERT INTO costRack (slots,costPerUnit) VALUES (43,430);
INSERT INTO costRack (slots,costPerUnit) VALUES (44,440);
INSERT INTO costRack (slots,costPerUnit) VALUES (45,450);
INSERT INTO costRack (slots,costPerUnit) VALUES (46,460);
INSERT INTO costRack (slots,costPerUnit) VALUES (47,470);
INSERT INTO costRack (slots,costPerUnit) VALUES (48,480);
INSERT INTO costRack (slots,costPerUnit) VALUES (49,490);
INSERT INTO costRack (slots,costPerUnit) VALUES (50,500);
INSERT INTO costRack (slots,costPerUnit) VALUES (51,510);
INSERT INTO costRack (slots,costPerUnit) VALUES (52,520);
INSERT INTO costRack (slots,costPerUnit) VALUES (53,530);
INSERT INTO costRack (slots,costPerUnit) VALUES (54,540);
INSERT INTO costRack (slots,costPerUnit) VALUES (55,550);
INSERT INTO costRack (slots,costPerUnit) VALUES (56,560);
INSERT INTO costRack (slots,costPerUnit) VALUES (57,570);
INSERT INTO costRack (slots,costPerUnit) VALUES (58,580);
...
INSERT INTO costService (type,rate,unit,lagMembers,costPerUnit) VALUES ('circuit',1,'Gbps',0,512.00);
INSERT INTO costService (type,rate,unit,lagMembers,costPerUnit) VALUES ('ethernet',1,'Gbps',0,384.12);
INSERT INTO costService (type,rate,unit,lagMembers,costPerUnit) VALUES ('fttx',1,'Gbps',0,256.78);
INSERT INTO costService (type,rate,unit,lagMembers,costPerUnit) VALUES ('voice',3,'Mbps',0,26.00);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('industrial','colo',2500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('industrial','dc',4000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('industrial','exchange',700);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('industrial','pop',1500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('industrial','street',3000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('industrial','unclassified',5000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('mixed','colo',2500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('mixed','dc',4000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('mixed','exchange',700);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('mixed','pop',1500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('mixed','street',3000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('mixed','unclassified',5000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('psz','colo',2500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('psz','exchange',4000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('psz','pop',1500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('psz','street',3000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('psz','unclassified',5000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('residential','street',500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('residential','unclassified',1000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('rural','colo',2500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('rural','exchange',4000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('rural','pop',1500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('rural','street',3000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('rural','unclassified',10000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('unclassified','unclassified',10000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('urban','colo',2500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('urban','exchange',4000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('urban','pop',1500);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('urban','street',3000);
INSERT INTO costSite (area,type,costPerUnit) VALUES ('urban','unclassified',5000);
INSERT INTO costTrench (purpose,unit,costPerUnit) VALUES ('backhaul','m',100.00);
INSERT INTO costTrench (purpose,unit,costPerUnit) VALUES ('pole','m',65.00);
INSERT INTO costTrench (purpose,unit,costPerUnit) VALUES ('ring','m',85.00);
INSERT INTO costTrench (purpose,unit,costPerUnit) VALUES ('service/drop','m',50.00);
INSERT INTO costTrench (purpose,unit,costPerUnit) VALUES ('tower','m',75.00);
...
-- World Geometries
--
CREATE TABLE IF NOT EXISTS world_ABW AS SELECT * FROM ST_Read('world/ABW.geojson');
CREATE TABLE IF NOT EXISTS world_AFG AS SELECT * FROM ST_Read('world/AFG.geojson');
CREATE TABLE IF NOT EXISTS world_AGO AS SELECT * FROM ST_Read('world/AGO.geojson');
CREATE TABLE IF NOT EXISTS world_AIA AS SELECT * FROM ST_Read('world/AIA.geojson');
CREATE TABLE IF NOT EXISTS world_ALB AS SELECT * FROM ST_Read('world/ALB.geojson');
CREATE TABLE IF NOT EXISTS world_AND AS SELECT * FROM ST_Read('world/AND.geojson');
CREATE TABLE IF NOT EXISTS world_ARE AS SELECT * FROM ST_Read('world/ARE.geojson');
CREATE TABLE IF NOT EXISTS world_ARG AS SELECT * FROM ST_Read('world/ARG.geojson');
CREATE TABLE IF NOT EXISTS world_ARM AS SELECT * FROM ST_Read('world/ARM.geojson');
CREATE TABLE IF NOT EXISTS world_ASM AS SELECT * FROM ST_Read('world/ASM.geojson');
CREATE TABLE IF NOT EXISTS world_ATA AS SELECT * FROM ST_Read('world/ATA.geojson');
CREATE TABLE IF NOT EXISTS world_ATG AS SELECT * FROM ST_Read('world/ATG.geojson');
CREATE TABLE IF NOT EXISTS world_AUS AS SELECT * FROM ST_Read('world/AUS.geojson');
CREATE TABLE IF NOT EXISTS world_AUT AS SELECT * FROM ST_Read('world/AUT.geojson');
CREATE TABLE IF NOT EXISTS world_AZE AS SELECT * FROM ST_Read('world/AZE.geojson');
CREATE TABLE IF NOT EXISTS world_BDI AS SELECT * FROM ST_Read('world/BDI.geojson');
CREATE TABLE IF NOT EXISTS world_BEL AS SELECT * FROM ST_Read('world/BEL.geojson');
CREATE TABLE IF NOT EXISTS world_BEN AS SELECT * FROM ST_Read('world/BEN.geojson');
CREATE TABLE IF NOT EXISTS world_BES AS SELECT * FROM ST_Read('world/BES.geojson');
CREATE TABLE IF NOT EXISTS world_BFA AS SELECT * FROM ST_Read('world/BFA.geojson');
CREATE TABLE IF NOT EXISTS world_BGD AS SELECT * FROM ST_Read('world/BGD.geojson');
CREATE TABLE IF NOT EXISTS world_BGR AS SELECT * FROM ST_Read('world/BGR.geojson');
CREATE TABLE IF NOT EXISTS world_BHR AS SELECT * FROM ST_Read('world/BHR.geojson');
CREATE TABLE IF NOT EXISTS world_BHS AS SELECT * FROM ST_Read('world/BHS.geojson');
CREATE TABLE IF NOT EXISTS world_BIH AS SELECT * FROM ST_Read('world/BIH.geojson');
CREATE TABLE IF NOT EXISTS world_BLM AS SELECT * FROM ST_Read('world/BLM.geojson');
CREATE TABLE IF NOT EXISTS world_BLR AS SELECT * FROM ST_Read('world/BLR.geojson');
CREATE TABLE IF NOT EXISTS world_BLZ AS SELECT * FROM ST_Read('world/BLZ.geojson');
CREATE TABLE IF NOT EXISTS world_BMU AS SELECT * FROM ST_Read('world/BMU.geojson');
CREATE TABLE IF NOT EXISTS world_BOL AS SELECT * FROM ST_Read('world/BOL.geojson');
CREATE TABLE IF NOT EXISTS world_BRA AS SELECT * FROM ST_Read('world/BRA.geojson');
CREATE TABLE IF NOT EXISTS world_BRB AS SELECT * FROM ST_Read('world/BRB.geojson');
CREATE TABLE IF NOT EXISTS world_BRN AS SELECT * FROM ST_Read('world/BRN.geojson');
CREATE TABLE IF NOT EXISTS world_BTN AS SELECT * FROM ST_Read('world/BTN.geojson');
CREATE TABLE IF NOT EXISTS world_BWA AS SELECT * FROM ST_Read('world/BWA.geojson');
CREATE TABLE IF NOT EXISTS world_CAF AS SELECT * FROM ST_Read('world/CAF.geojson');
CREATE TABLE IF NOT EXISTS world_CAN AS SELECT * FROM ST_Read('world/CAN.geojson');
CREATE TABLE IF NOT EXISTS world_CHE AS SELECT * FROM ST_Read('world/CHE.geojson');
CREATE TABLE IF NOT EXISTS world_CHL AS SELECT * FROM ST_Read('world/CHL.geojson');
CREATE TABLE IF NOT EXISTS world_CHN AS SELECT * FROM ST_Read('world/CHN.geojson');
CREATE TABLE IF NOT EXISTS world_CIV AS SELECT * FROM ST_Read('world/CIV.geojson');
CREATE TABLE IF NOT EXISTS world_CMR AS SELECT * FROM ST_Read('world/CMR.geojson');
CREATE TABLE IF NOT EXISTS world_COD AS SELECT * FROM ST_Read('world/COD.geojson');
CREATE TABLE IF NOT EXISTS world_COG AS SELECT * FROM ST_Read('world/COG.geojson');
CREATE TABLE IF NOT EXISTS world_COK AS SELECT * FROM ST_Read('world/COK.geojson');
CREATE TABLE IF NOT EXISTS world_COL AS SELECT * FROM ST_Read('world/COL.geojson');
CREATE TABLE IF NOT EXISTS world_COM AS SELECT * FROM ST_Read('world/COM.geojson');
CREATE TABLE IF NOT EXISTS world_CPV AS SELECT * FROM ST_Read('world/CPV.geojson');
CREATE TABLE IF NOT EXISTS world_CRI AS SELECT * FROM ST_Read('world/CRI.geojson');
CREATE TABLE IF NOT EXISTS world_CUB AS SELECT * FROM ST_Read('world/CUB.geojson');
CREATE TABLE IF NOT EXISTS world_CUW AS SELECT * FROM ST_Read('world/CUW.geojson');
CREATE TABLE IF NOT EXISTS world_CYM AS SELECT * FROM ST_Read('world/CYM.geojson');
CREATE TABLE IF NOT EXISTS world_CYP AS SELECT * FROM ST_Read('world/CYP.geojson');
CREATE TABLE IF NOT EXISTS world_CZE AS SELECT * FROM ST_Read('world/CZE.geojson');
CREATE TABLE IF NOT EXISTS world_DEU AS SELECT * FROM ST_Read('world/DEU.geojson');
CREATE TABLE IF NOT EXISTS world_DJI AS SELECT * FROM ST_Read('world/DJI.geojson');
CREATE TABLE IF NOT EXISTS world_DMA AS SELECT * FROM ST_Read('world/DMA.geojson');
CREATE TABLE IF NOT EXISTS world_DNK AS SELECT * FROM ST_Read('world/DNK.geojson');
CREATE TABLE IF NOT EXISTS world_DOM AS SELECT * FROM ST_Read('world/DOM.geojson');
CREATE TABLE IF NOT EXISTS world_DZA AS SELECT * FROM ST_Read('world/DZA.geojson');
CREATE TABLE IF NOT EXISTS world_ECU AS SELECT * FROM ST_Read('world/ECU.geojson');
CREATE TABLE IF NOT EXISTS world_EGY AS SELECT * FROM ST_Read('world/EGY.geojson');
CREATE TABLE IF NOT EXISTS world_ERI AS SELECT * FROM ST_Read('world/ERI.geojson');
CREATE TABLE IF NOT EXISTS world_ESP AS SELECT * FROM ST_Read('world/ESP.geojson');
CREATE TABLE IF NOT EXISTS world_EST AS SELECT * FROM ST_Read('world/EST.geojson');
CREATE TABLE IF NOT EXISTS world_ETH AS SELECT * FROM ST_Read('world/ETH.geojson');
CREATE TABLE IF NOT EXISTS world_FIN AS SELECT * FROM ST_Read('world/FIN.geojson');
CREATE TABLE IF NOT EXISTS world_FJI AS SELECT * FROM ST_Read('world/FJI.geojson');
CREATE TABLE IF NOT EXISTS world_FLK AS SELECT * FROM ST_Read('world/FLK.geojson');
CREATE TABLE IF NOT EXISTS world_FRA AS SELECT * FROM ST_Read('world/FRA.geojson');
CREATE TABLE IF NOT EXISTS world_FRO AS SELECT * FROM ST_Read('world/FRO.geojson');
CREATE TABLE IF NOT EXISTS world_FSM AS SELECT * FROM ST_Read('world/FSM.geojson');
CREATE TABLE IF NOT EXISTS world_GAB AS SELECT * FROM ST_Read('world/GAB.geojson');
CREATE TABLE IF NOT EXISTS world_GBR AS SELECT * FROM ST_Read('world/GBR.geojson');
CREATE TABLE IF NOT EXISTS world_GEO AS SELECT * FROM ST_Read('world/GEO.geojson');
CREATE TABLE IF NOT EXISTS world_GGY AS SELECT * FROM ST_Read('world/GGY.geojson');
CREATE TABLE IF NOT EXISTS world_GHA AS SELECT * FROM ST_Read('world/GHA.geojson');
CREATE TABLE IF NOT EXISTS world_GIB AS SELECT * FROM ST_Read('world/GIB.geojson');
CREATE TABLE IF NOT EXISTS world_GIN AS SELECT * FROM ST_Read('world/GIN.geojson');
CREATE TABLE IF NOT EXISTS world_GLP AS SELECT * FROM ST_Read('world/GLP.geojson');
CREATE TABLE IF NOT EXISTS world_GMB AS SELECT * FROM ST_Read('world/GMB.geojson');
CREATE TABLE IF NOT EXISTS world_GNB AS SELECT * FROM ST_Read('world/GNB.geojson');
CREATE TABLE IF NOT EXISTS world_GNQ AS SELECT * FROM ST_Read('world/GNQ.geojson');
CREATE TABLE IF NOT EXISTS world_GRC AS SELECT * FROM ST_Read('world/GRC.geojson');
CREATE TABLE IF NOT EXISTS world_GRD AS SELECT * FROM ST_Read('world/GRD.geojson');
CREATE TABLE IF NOT EXISTS world_GRL AS SELECT * FROM ST_Read('world/GRL.geojson');
CREATE TABLE IF NOT EXISTS world_GTM AS SELECT * FROM ST_Read('world/GTM.geojson');
CREATE TABLE IF NOT EXISTS world_GUF AS SELECT * FROM ST_Read('world/GUF.geojson');
CREATE TABLE IF NOT EXISTS world_GUM AS SELECT * FROM ST_Read('world/GUM.geojson');
CREATE TABLE IF NOT EXISTS world_GUY AS SELECT * FROM ST_Read('world/GUY.geojson');
CREATE TABLE IF NOT EXISTS world_HND AS SELECT * FROM ST_Read('world/HND.geojson');
CREATE TABLE IF NOT EXISTS world_HRV AS SELECT * FROM ST_Read('world/HRV.geojson');
CREATE TABLE IF NOT EXISTS world_HTI AS SELECT * FROM ST_Read('world/HTI.geojson');
CREATE TABLE IF NOT EXISTS world_HUN AS SELECT * FROM ST_Read('world/HUN.geojson');
CREATE TABLE IF NOT EXISTS world_IDN AS SELECT * FROM ST_Read('world/IDN.geojson');
CREATE TABLE IF NOT EXISTS world_IMN AS SELECT * FROM ST_Read('world/IMN.geojson');
CREATE TABLE IF NOT EXISTS world_IND AS SELECT * FROM ST_Read('world/IND.geojson');
CREATE TABLE IF NOT EXISTS world_IRL AS SELECT * FROM ST_Read('world/IRL.geojson');
CREATE TABLE IF NOT EXISTS world_IRN AS SELECT * FROM ST_Read('world/IRN.geojson');
CREATE TABLE IF NOT EXISTS world_IRQ AS SELECT * FROM ST_Read('world/IRQ.geojson');
CREATE TABLE IF NOT EXISTS world_ISL AS SELECT * FROM ST_Read('world/ISL.geojson');
CREATE TABLE IF NOT EXISTS world_ISR AS SELECT * FROM ST_Read('world/ISR.geojson');
CREATE TABLE IF NOT EXISTS world_ITA AS SELECT * FROM ST_Read('world/ITA.geojson');
CREATE TABLE IF NOT EXISTS world_JAM AS SELECT * FROM ST_Read('world/JAM.geojson');
CREATE TABLE IF NOT EXISTS world_JOR AS SELECT * FROM ST_Read('world/JOR.geojson');
CREATE TABLE IF NOT EXISTS world_JPN AS SELECT * FROM ST_Read('world/JPN.geojson');
CREATE TABLE IF NOT EXISTS world_KAZ AS SELECT * FROM ST_Read('world/KAZ.geojson');
CREATE TABLE IF NOT EXISTS world_KEN AS SELECT * FROM ST_Read('world/KEN.geojson');
CREATE TABLE IF NOT EXISTS world_KGZ AS SELECT * FROM ST_Read('world/KGZ.geojson');
CREATE TABLE IF NOT EXISTS world_KHM AS SELECT * FROM ST_Read('world/KHM.geojson');
CREATE TABLE IF NOT EXISTS world_KIR AS SELECT * FROM ST_Read('world/KIR.geojson');
CREATE TABLE IF NOT EXISTS world_KNA AS SELECT * FROM ST_Read('world/KNA.geojson');
CREATE TABLE IF NOT EXISTS world_KOR AS SELECT * FROM ST_Read('world/KOR.geojson');
CREATE TABLE IF NOT EXISTS world_KWT AS SELECT * FROM ST_Read('world/KWT.geojson');
CREATE TABLE IF NOT EXISTS world_LAO AS SELECT * FROM ST_Read('world/LAO.geojson');
CREATE TABLE IF NOT EXISTS world_LBN AS SELECT * FROM ST_Read('world/LBN.geojson');
CREATE TABLE IF NOT EXISTS world_LBR AS SELECT * FROM ST_Read('world/LBR.geojson');
CREATE TABLE IF NOT EXISTS world_LBY AS SELECT * FROM ST_Read('world/LBY.geojson');
CREATE TABLE IF NOT EXISTS world_LCA AS SELECT * FROM ST_Read('world/LCA.geojson');
CREATE TABLE IF NOT EXISTS world_LIE AS SELECT * FROM ST_Read('world/LIE.geojson');
CREATE TABLE IF NOT EXISTS world_LKA AS SELECT * FROM ST_Read('world/LKA.geojson');
CREATE TABLE IF NOT EXISTS world_LSO AS SELECT * FROM ST_Read('world/LSO.geojson');
CREATE TABLE IF NOT EXISTS world_LTU AS SELECT * FROM ST_Read('world/LTU.geojson');
CREATE TABLE IF NOT EXISTS world_LUX AS SELECT * FROM ST_Read('world/LUX.geojson');
CREATE TABLE IF NOT EXISTS world_LVA AS SELECT * FROM ST_Read('world/LVA.geojson');
CREATE TABLE IF NOT EXISTS world_MAR AS SELECT * FROM ST_Read('world/MAR.geojson');
CREATE TABLE IF NOT EXISTS world_MCO AS SELECT * FROM ST_Read('world/MCO.geojson');
CREATE TABLE IF NOT EXISTS world_MDA AS SELECT * FROM ST_Read('world/MDA.geojson');
CREATE TABLE IF NOT EXISTS world_MDG AS SELECT * FROM ST_Read('world/MDG.geojson');
CREATE TABLE IF NOT EXISTS world_MDV AS SELECT * FROM ST_Read('world/MDV.geojson');
CREATE TABLE IF NOT EXISTS world_MEX AS SELECT * FROM ST_Read('world/MEX.geojson');
CREATE TABLE IF NOT EXISTS world_MHL AS SELECT * FROM ST_Read('world/MHL.geojson');
CREATE TABLE IF NOT EXISTS world_MKD AS SELECT * FROM ST_Read('world/MKD.geojson');
CREATE TABLE IF NOT EXISTS world_MLI AS SELECT * FROM ST_Read('world/MLI.geojson');
CREATE TABLE IF NOT EXISTS world_MLT AS SELECT * FROM ST_Read('world/MLT.geojson');
CREATE TABLE IF NOT EXISTS world_MMR AS SELECT * FROM ST_Read('world/MMR.geojson');
CREATE TABLE IF NOT EXISTS world_MNE AS SELECT * FROM ST_Read('world/MNE.geojson');
CREATE TABLE IF NOT EXISTS world_MNG AS SELECT * FROM ST_Read('world/MNG.geojson');
CREATE TABLE IF NOT EXISTS world_MNP AS SELECT * FROM ST_Read('world/MNP.geojson');
CREATE TABLE IF NOT EXISTS world_MOZ AS SELECT * FROM ST_Read('world/MOZ.geojson');
CREATE TABLE IF NOT EXISTS world_MRT AS SELECT * FROM ST_Read('world/MRT.geojson');
CREATE TABLE IF NOT EXISTS world_MSR AS SELECT * FROM ST_Read('world/MSR.geojson');
CREATE TABLE IF NOT EXISTS world_MTQ AS SELECT * FROM ST_Read('world/MTQ.geojson');
CREATE TABLE IF NOT EXISTS world_MUS AS SELECT * FROM ST_Read('world/MUS.geojson');
CREATE TABLE IF NOT EXISTS world_MWI AS SELECT * FROM ST_Read('world/MWI.geojson');
CREATE TABLE IF NOT EXISTS world_MYS AS SELECT * FROM ST_Read('world/MYS.geojson');
CREATE TABLE IF NOT EXISTS world_MYT AS SELECT * FROM ST_Read('world/MYT.geojson');
CREATE TABLE IF NOT EXISTS world_NAM AS SELECT * FROM ST_Read('world/NAM.geojson');
CREATE TABLE IF NOT EXISTS world_NCL AS SELECT * FROM ST_Read('world/NCL.geojson');
CREATE TABLE IF NOT EXISTS world_NER AS SELECT * FROM ST_Read('world/NER.geojson');
CREATE TABLE IF NOT EXISTS world_NGA AS SELECT * FROM ST_Read('world/NGA.geojson');
CREATE TABLE IF NOT EXISTS world_NIC AS SELECT * FROM ST_Read('world/NIC.geojson');
CREATE TABLE IF NOT EXISTS world_NIU AS SELECT * FROM ST_Read('world/NIU.geojson');
CREATE TABLE IF NOT EXISTS world_NLD AS SELECT * FROM ST_Read('world/NLD.geojson');
CREATE TABLE IF NOT EXISTS world_NOR AS SELECT * FROM ST_Read('world/NOR.geojson');
CREATE TABLE IF NOT EXISTS world_NPL AS SELECT * FROM ST_Read('world/NPL.geojson');
CREATE TABLE IF NOT EXISTS world_NRU AS SELECT * FROM ST_Read('world/NRU.geojson');
CREATE TABLE IF NOT EXISTS world_NZL AS SELECT * FROM ST_Read('world/NZL.geojson');
CREATE TABLE IF NOT EXISTS world_OMN AS SELECT * FROM ST_Read('world/OMN.geojson');
CREATE TABLE IF NOT EXISTS world_PAK AS SELECT * FROM ST_Read('world/PAK.geojson');
CREATE TABLE IF NOT EXISTS world_PAN AS SELECT * FROM ST_Read('world/PAN.geojson');
CREATE TABLE IF NOT EXISTS world_PCN AS SELECT * FROM ST_Read('world/PCN.geojson');
CREATE TABLE IF NOT EXISTS world_PER AS SELECT * FROM ST_Read('world/PER.geojson');
CREATE TABLE IF NOT EXISTS world_PHL AS SELECT * FROM ST_Read('world/PHL.geojson');
CREATE TABLE IF NOT EXISTS world_PLW AS SELECT * FROM ST_Read('world/PLW.geojson');
CREATE TABLE IF NOT EXISTS world_PNG AS SELECT * FROM ST_Read('world/PNG.geojson');
CREATE TABLE IF NOT EXISTS world_POL AS SELECT * FROM ST_Read('world/POL.geojson');
CREATE TABLE IF NOT EXISTS world_PRK AS SELECT * FROM ST_Read('world/PRK.geojson');
CREATE TABLE IF NOT EXISTS world_PRT AS SELECT * FROM ST_Read('world/PRT.geojson');
CREATE TABLE IF NOT EXISTS world_PRY AS SELECT * FROM ST_Read('world/PRY.geojson');
CREATE TABLE IF NOT EXISTS world_PSE AS SELECT * FROM ST_Read('world/PSE.geojson');
CREATE TABLE IF NOT EXISTS world_PYF AS SELECT * FROM ST_Read('world/PYF.geojson');
CREATE TABLE IF NOT EXISTS world_QAT AS SELECT * FROM ST_Read('world/QAT.geojson');
CREATE TABLE IF NOT EXISTS world_REU AS SELECT * FROM ST_Read('world/REU.geojson');
CREATE TABLE IF NOT EXISTS world_ROU AS SELECT * FROM ST_Read('world/ROU.geojson');
CREATE TABLE IF NOT EXISTS world_RUS AS SELECT * FROM ST_Read('world/RUS.geojson');
CREATE TABLE IF NOT EXISTS world_RWA AS SELECT * FROM ST_Read('world/RWA.geojson');
CREATE TABLE IF NOT EXISTS world_SAU AS SELECT * FROM ST_Read('world/SAU.geojson');
CREATE TABLE IF NOT EXISTS world_SDN AS SELECT * FROM ST_Read('world/SDN.geojson');
CREATE TABLE IF NOT EXISTS world_SEN AS SELECT * FROM ST_Read('world/SEN.geojson');
CREATE TABLE IF NOT EXISTS world_SGP AS SELECT * FROM ST_Read('world/SGP.geojson');
CREATE TABLE IF NOT EXISTS world_SHN AS SELECT * FROM ST_Read('world/SHN.geojson');
CREATE TABLE IF NOT EXISTS world_SLB AS SELECT * FROM ST_Read('world/SLB.geojson');
CREATE TABLE IF NOT EXISTS world_SLE AS SELECT * FROM ST_Read('world/SLE.geojson');
CREATE TABLE IF NOT EXISTS world_SLV AS SELECT * FROM ST_Read('world/SLV.geojson');
CREATE TABLE IF NOT EXISTS world_SMR AS SELECT * FROM ST_Read('world/SMR.geojson');
CREATE TABLE IF NOT EXISTS world_SOM AS SELECT * FROM ST_Read('world/SOM.geojson');
CREATE TABLE IF NOT EXISTS world_SRB AS SELECT * FROM ST_Read('world/SRB.geojson');
CREATE TABLE IF NOT EXISTS world_SSD AS SELECT * FROM ST_Read('world/SSD.geojson');
CREATE TABLE IF NOT EXISTS world_STP AS SELECT * FROM ST_Read('world/STP.geojson');
CREATE TABLE IF NOT EXISTS world_SUR AS SELECT * FROM ST_Read('world/SUR.geojson');
CREATE TABLE IF NOT EXISTS world_SVK AS SELECT * FROM ST_Read('world/SVK.geojson');
CREATE TABLE IF NOT EXISTS world_SVN AS SELECT * FROM ST_Read('world/SVN.geojson');
CREATE TABLE IF NOT EXISTS world_SWE AS SELECT * FROM ST_Read('world/SWE.geojson');
CREATE TABLE IF NOT EXISTS world_SWZ AS SELECT * FROM ST_Read('world/SWZ.geojson');
CREATE TABLE IF NOT EXISTS world_SYC AS SELECT * FROM ST_Read('world/SYC.geojson');
CREATE TABLE IF NOT EXISTS world_SYR AS SELECT * FROM ST_Read('world/SYR.geojson');
CREATE TABLE IF NOT EXISTS world_TCA AS SELECT * FROM ST_Read('world/TCA.geojson');
CREATE TABLE IF NOT EXISTS world_TCD AS SELECT * FROM ST_Read('world/TCD.geojson');
CREATE TABLE IF NOT EXISTS world_TGO AS SELECT * FROM ST_Read('world/TGO.geojson');
CREATE TABLE IF NOT EXISTS world_THA AS SELECT * FROM ST_Read('world/THA.geojson');
CREATE TABLE IF NOT EXISTS world_TJK AS SELECT * FROM ST_Read('world/TJK.geojson');
CREATE TABLE IF NOT EXISTS world_TKL AS SELECT * FROM ST_Read('world/TKL.geojson');
CREATE TABLE IF NOT EXISTS world_TKM AS SELECT * FROM ST_Read('world/TKM.geojson');
CREATE TABLE IF NOT EXISTS world_TLS AS SELECT * FROM ST_Read('world/TLS.geojson');
CREATE TABLE IF NOT EXISTS world_TON AS SELECT * FROM ST_Read('world/TON.geojson');
CREATE TABLE IF NOT EXISTS world_TTO AS SELECT * FROM ST_Read('world/TTO.geojson');
CREATE TABLE IF NOT EXISTS world_TUN AS SELECT * FROM ST_Read('world/TUN.geojson');
CREATE TABLE IF NOT EXISTS world_TUR AS SELECT * FROM ST_Read('world/TUR.geojson');
CREATE TABLE IF NOT EXISTS world_TUV AS SELECT * FROM ST_Read('world/TUV.geojson');
CREATE TABLE IF NOT EXISTS world_TWN AS SELECT * FROM ST_Read('world/TWN.geojson');
CREATE TABLE IF NOT EXISTS world_TZA AS SELECT * FROM ST_Read('world/TZA.geojson');
CREATE TABLE IF NOT EXISTS world_UGA AS SELECT * FROM ST_Read('world/UGA.geojson');
CREATE TABLE IF NOT EXISTS world_UKR AS SELECT * FROM ST_Read('world/UKR.geojson');
CREATE TABLE IF NOT EXISTS world_URY AS SELECT * FROM ST_Read('world/URY.geojson');
CREATE TABLE IF NOT EXISTS world_USA AS SELECT * FROM ST_Read('world/USA.geojson');
CREATE TABLE IF NOT EXISTS world_UZB AS SELECT * FROM ST_Read('world/UZB.geojson');
CREATE TABLE IF NOT EXISTS world_VAT AS SELECT * FROM ST_Read('world/VAT.geojson');
CREATE TABLE IF NOT EXISTS world_VCT AS SELECT * FROM ST_Read('world/VCT.geojson');
CREATE TABLE IF NOT EXISTS world_VEN AS SELECT * FROM ST_Read('world/VEN.geojson');
CREATE TABLE IF NOT EXISTS world_VGB AS SELECT * FROM ST_Read('world/VGB.geojson');
CREATE TABLE IF NOT EXISTS world_VIR AS SELECT * FROM ST_Read('world/VIR.geojson');
CREATE TABLE IF NOT EXISTS world_VNM AS SELECT * FROM ST_Read('world/VNM.geojson');
CREATE TABLE IF NOT EXISTS world_VUT AS SELECT * FROM ST_Read('world/VUT.geojson');
CREATE TABLE IF NOT EXISTS world_WLF AS SELECT * FROM ST_Read('world/WLF.geojson');
CREATE TABLE IF NOT EXISTS world_WSM AS SELECT * FROM ST_Read('world/WSM.geojson');
CREATE TABLE IF NOT EXISTS world_YEM AS SELECT * FROM ST_Read('world/YEM.geojson');
CREATE TABLE IF NOT EXISTS world_ZAF AS SELECT * FROM ST_Read('world/ZAF.geojson');
CREATE TABLE IF NOT EXISTS world_ZMB AS SELECT * FROM ST_Read('world/ZMB.geojson');
CREATE TABLE IF NOT EXISTS world_ZWE AS SELECT * FROM ST_Read('world/ZWE.geojson');

BEGIN TRANSACTION;
DELETE FROM worldGeo;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ABW',geom FROM world_ABW;
INSERT INTO worldGeo (country,geoCollection) SELECT 'AFG',geom FROM world_AFG;
INSERT INTO worldGeo (country,geoCollection) SELECT 'AGO',geom FROM world_AGO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'AIA',geom FROM world_AIA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ALB',geom FROM world_ALB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'AND',geom FROM world_AND;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ARE',geom FROM world_ARE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ARG',geom FROM world_ARG;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ARM',geom FROM world_ARM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ASM',geom FROM world_ASM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ATA',geom FROM world_ATA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ATG',geom FROM world_ATG;
INSERT INTO worldGeo (country,geoCollection) SELECT 'AUS',geom FROM world_AUS;
INSERT INTO worldGeo (country,geoCollection) SELECT 'AUT',geom FROM world_AUT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'AZE',geom FROM world_AZE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BDI',geom FROM world_BDI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BEL',geom FROM world_BEL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BEN',geom FROM world_BEN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BES',geom FROM world_BES;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BFA',geom FROM world_BFA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BGD',geom FROM world_BGD;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BGR',geom FROM world_BGR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BHR',geom FROM world_BHR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BHS',geom FROM world_BHS;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BIH',geom FROM world_BIH;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BLM',geom FROM world_BLM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BLR',geom FROM world_BLR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BLZ',geom FROM world_BLZ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BMU',geom FROM world_BMU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BOL',geom FROM world_BOL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BRA',geom FROM world_BRA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BRB',geom FROM world_BRB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BRN',geom FROM world_BRN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BTN',geom FROM world_BTN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'BWA',geom FROM world_BWA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CAF',geom FROM world_CAF;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CAN',geom FROM world_CAN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CHE',geom FROM world_CHE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CHL',geom FROM world_CHL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CHN',geom FROM world_CHN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CIV',geom FROM world_CIV;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CMR',geom FROM world_CMR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'COD',geom FROM world_COD;
INSERT INTO worldGeo (country,geoCollection) SELECT 'COG',geom FROM world_COG;
INSERT INTO worldGeo (country,geoCollection) SELECT 'COK',geom FROM world_COK;
INSERT INTO worldGeo (country,geoCollection) SELECT 'COL',geom FROM world_COL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'COM',geom FROM world_COM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CPV',geom FROM world_CPV;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CRI',geom FROM world_CRI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CUB',geom FROM world_CUB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CUW',geom FROM world_CUW;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CYM',geom FROM world_CYM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CYP',geom FROM world_CYP;
INSERT INTO worldGeo (country,geoCollection) SELECT 'CZE',geom FROM world_CZE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'DEU',geom FROM world_DEU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'DJI',geom FROM world_DJI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'DMA',geom FROM world_DMA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'DNK',geom FROM world_DNK;
INSERT INTO worldGeo (country,geoCollection) SELECT 'DOM',geom FROM world_DOM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'DZA',geom FROM world_DZA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ECU',geom FROM world_ECU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'EGY',geom FROM world_EGY;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ERI',geom FROM world_ERI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ESP',geom FROM world_ESP;
INSERT INTO worldGeo (country,geoCollection) SELECT 'EST',geom FROM world_EST;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ETH',geom FROM world_ETH;
INSERT INTO worldGeo (country,geoCollection) SELECT 'FIN',geom FROM world_FIN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'FJI',geom FROM world_FJI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'FLK',geom FROM world_FLK;
INSERT INTO worldGeo (country,geoCollection) SELECT 'FRA',geom FROM world_FRA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'FRO',geom FROM world_FRO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'FSM',geom FROM world_FSM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GAB',geom FROM world_GAB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GBR',geom FROM world_GBR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GEO',geom FROM world_GEO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GGY',geom FROM world_GGY;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GHA',geom FROM world_GHA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GIB',geom FROM world_GIB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GIN',geom FROM world_GIN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GLP',geom FROM world_GLP;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GMB',geom FROM world_GMB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GNB',geom FROM world_GNB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GNQ',geom FROM world_GNQ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GRC',geom FROM world_GRC;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GRD',geom FROM world_GRD;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GRL',geom FROM world_GRL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GTM',geom FROM world_GTM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GUF',geom FROM world_GUF;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GUM',geom FROM world_GUM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'GUY',geom FROM world_GUY;
INSERT INTO worldGeo (country,geoCollection) SELECT 'HND',geom FROM world_HND;
INSERT INTO worldGeo (country,geoCollection) SELECT 'HRV',geom FROM world_HRV;
INSERT INTO worldGeo (country,geoCollection) SELECT 'HTI',geom FROM world_HTI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'HUN',geom FROM world_HUN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'IDN',geom FROM world_IDN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'IMN',geom FROM world_IMN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'IND',geom FROM world_IND;
INSERT INTO worldGeo (country,geoCollection) SELECT 'IRL',geom FROM world_IRL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'IRN',geom FROM world_IRN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'IRQ',geom FROM world_IRQ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ISL',geom FROM world_ISL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ISR',geom FROM world_ISR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ITA',geom FROM world_ITA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'JAM',geom FROM world_JAM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'JOR',geom FROM world_JOR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'JPN',geom FROM world_JPN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KAZ',geom FROM world_KAZ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KEN',geom FROM world_KEN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KGZ',geom FROM world_KGZ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KHM',geom FROM world_KHM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KIR',geom FROM world_KIR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KNA',geom FROM world_KNA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KOR',geom FROM world_KOR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'KWT',geom FROM world_KWT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LAO',geom FROM world_LAO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LBN',geom FROM world_LBN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LBR',geom FROM world_LBR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LBY',geom FROM world_LBY;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LCA',geom FROM world_LCA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LIE',geom FROM world_LIE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LKA',geom FROM world_LKA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LSO',geom FROM world_LSO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LTU',geom FROM world_LTU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LUX',geom FROM world_LUX;
INSERT INTO worldGeo (country,geoCollection) SELECT 'LVA',geom FROM world_LVA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MAR',geom FROM world_MAR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MCO',geom FROM world_MCO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MDA',geom FROM world_MDA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MDG',geom FROM world_MDG;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MDV',geom FROM world_MDV;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MEX',geom FROM world_MEX;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MHL',geom FROM world_MHL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MKD',geom FROM world_MKD;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MLI',geom FROM world_MLI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MLT',geom FROM world_MLT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MMR',geom FROM world_MMR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MNE',geom FROM world_MNE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MNG',geom FROM world_MNG;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MNP',geom FROM world_MNP;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MOZ',geom FROM world_MOZ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MRT',geom FROM world_MRT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MSR',geom FROM world_MSR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MTQ',geom FROM world_MTQ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MUS',geom FROM world_MUS;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MWI',geom FROM world_MWI;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MYS',geom FROM world_MYS;
INSERT INTO worldGeo (country,geoCollection) SELECT 'MYT',geom FROM world_MYT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NAM',geom FROM world_NAM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NCL',geom FROM world_NCL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NER',geom FROM world_NER;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NGA',geom FROM world_NGA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NIC',geom FROM world_NIC;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NIU',geom FROM world_NIU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NLD',geom FROM world_NLD;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NOR',geom FROM world_NOR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NPL',geom FROM world_NPL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NRU',geom FROM world_NRU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'NZL',geom FROM world_NZL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'OMN',geom FROM world_OMN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PAK',geom FROM world_PAK;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PAN',geom FROM world_PAN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PCN',geom FROM world_PCN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PER',geom FROM world_PER;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PHL',geom FROM world_PHL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PLW',geom FROM world_PLW;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PNG',geom FROM world_PNG;
INSERT INTO worldGeo (country,geoCollection) SELECT 'POL',geom FROM world_POL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PRK',geom FROM world_PRK;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PRT',geom FROM world_PRT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PRY',geom FROM world_PRY;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PSE',geom FROM world_PSE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'PYF',geom FROM world_PYF;
INSERT INTO worldGeo (country,geoCollection) SELECT 'QAT',geom FROM world_QAT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'REU',geom FROM world_REU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ROU',geom FROM world_ROU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'RUS',geom FROM world_RUS;
INSERT INTO worldGeo (country,geoCollection) SELECT 'RWA',geom FROM world_RWA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SAU',geom FROM world_SAU;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SDN',geom FROM world_SDN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SEN',geom FROM world_SEN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SGP',geom FROM world_SGP;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SHN',geom FROM world_SHN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SLB',geom FROM world_SLB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SLE',geom FROM world_SLE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SLV',geom FROM world_SLV;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SMR',geom FROM world_SMR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SOM',geom FROM world_SOM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SRB',geom FROM world_SRB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SSD',geom FROM world_SSD;
INSERT INTO worldGeo (country,geoCollection) SELECT 'STP',geom FROM world_STP;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SUR',geom FROM world_SUR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SVK',geom FROM world_SVK;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SVN',geom FROM world_SVN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SWE',geom FROM world_SWE;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SWZ',geom FROM world_SWZ;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SYC',geom FROM world_SYC;
INSERT INTO worldGeo (country,geoCollection) SELECT 'SYR',geom FROM world_SYR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TCA',geom FROM world_TCA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TCD',geom FROM world_TCD;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TGO',geom FROM world_TGO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'THA',geom FROM world_THA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TJK',geom FROM world_TJK;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TKL',geom FROM world_TKL;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TKM',geom FROM world_TKM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TLS',geom FROM world_TLS;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TON',geom FROM world_TON;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TTO',geom FROM world_TTO;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TUN',geom FROM world_TUN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TUR',geom FROM world_TUR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TUV',geom FROM world_TUV;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TWN',geom FROM world_TWN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'TZA',geom FROM world_TZA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'UGA',geom FROM world_UGA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'UKR',geom FROM world_UKR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'URY',geom FROM world_URY;
INSERT INTO worldGeo (country,geoCollection) SELECT 'USA',geom FROM world_USA;
INSERT INTO worldGeo (country,geoCollection) SELECT 'UZB',geom FROM world_UZB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'VAT',geom FROM world_VAT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'VCT',geom FROM world_VCT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'VEN',geom FROM world_VEN;
INSERT INTO worldGeo (country,geoCollection) SELECT 'VGB',geom FROM world_VGB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'VIR',geom FROM world_VIR;
INSERT INTO worldGeo (country,geoCollection) SELECT 'VNM',geom FROM world_VNM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'VUT',geom FROM world_VUT;
INSERT INTO worldGeo (country,geoCollection) SELECT 'WLF',geom FROM world_WLF;
INSERT INTO worldGeo (country,geoCollection) SELECT 'WSM',geom FROM world_WSM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'YEM',geom FROM world_YEM;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ZAF',geom FROM world_ZAF;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ZMB',geom FROM world_ZMB;
INSERT INTO worldGeo (country,geoCollection) SELECT 'ZWE',geom FROM world_ZWE;
COMMIT;

DROP TABLE world_ABW;
DROP TABLE world_AFG;
DROP TABLE world_AGO;
DROP TABLE world_AIA;
DROP TABLE world_ALB;
DROP TABLE world_AND;
DROP TABLE world_ARE;
DROP TABLE world_ARG;
DROP TABLE world_ARM;
DROP TABLE world_ASM;
DROP TABLE world_ATA;
DROP TABLE world_ATG;
DROP TABLE world_AUS;
DROP TABLE world_AUT;
DROP TABLE world_AZE;
DROP TABLE world_BDI;
DROP TABLE world_BEL;
DROP TABLE world_BEN;
DROP TABLE world_BES;
DROP TABLE world_BFA;
DROP TABLE world_BGD;
DROP TABLE world_BGR;
DROP TABLE world_BHR;
DROP TABLE world_BHS;
DROP TABLE world_BIH;
DROP TABLE world_BLM;
DROP TABLE world_BLR;
DROP TABLE world_BLZ;
DROP TABLE world_BMU;
DROP TABLE world_BOL;
DROP TABLE world_BRA;
DROP TABLE world_BRB;
DROP TABLE world_BRN;
DROP TABLE world_BTN;
DROP TABLE world_BWA;
DROP TABLE world_CAF;
DROP TABLE world_CAN;
DROP TABLE world_CHE;
DROP TABLE world_CHL;
DROP TABLE world_CHN;
DROP TABLE world_CIV;
DROP TABLE world_CMR;
DROP TABLE world_COD;
DROP TABLE world_COG;
DROP TABLE world_COK;
DROP TABLE world_COL;
DROP TABLE world_COM;
DROP TABLE world_CPV;
DROP TABLE world_CRI;
DROP TABLE world_CUB;
DROP TABLE world_CUW;
DROP TABLE world_CYM;
DROP TABLE world_CYP;
DROP TABLE world_CZE;
DROP TABLE world_DEU;
DROP TABLE world_DJI;
DROP TABLE world_DMA;
DROP TABLE world_DNK;
DROP TABLE world_DOM;
DROP TABLE world_DZA;
DROP TABLE world_ECU;
DROP TABLE world_EGY;
DROP TABLE world_ERI;
DROP TABLE world_ESP;
DROP TABLE world_EST;
DROP TABLE world_ETH;
DROP TABLE world_FIN;
DROP TABLE world_FJI;
DROP TABLE world_FLK;
DROP TABLE world_FRA;
DROP TABLE world_FRO;
DROP TABLE world_FSM;
DROP TABLE world_GAB;
DROP TABLE world_GBR;
DROP TABLE world_GEO;
DROP TABLE world_GGY;
DROP TABLE world_GHA;
DROP TABLE world_GIB;
DROP TABLE world_GIN;
DROP TABLE world_GLP;
DROP TABLE world_GMB;
DROP TABLE world_GNB;
DROP TABLE world_GNQ;
DROP TABLE world_GRC;
DROP TABLE world_GRD;
DROP TABLE world_GRL;
DROP TABLE world_GTM;
DROP TABLE world_GUF;
DROP TABLE world_GUM;
DROP TABLE world_GUY;
DROP TABLE world_HND;
DROP TABLE world_HRV;
DROP TABLE world_HTI;
DROP TABLE world_HUN;
DROP TABLE world_IDN;
DROP TABLE world_IMN;
DROP TABLE world_IND;
DROP TABLE world_IRL;
DROP TABLE world_IRN;
DROP TABLE world_IRQ;
DROP TABLE world_ISL;
DROP TABLE world_ISR;
DROP TABLE world_ITA;
DROP TABLE world_JAM;
DROP TABLE world_JOR;
DROP TABLE world_JPN;
DROP TABLE world_KAZ;
DROP TABLE world_KEN;
DROP TABLE world_KGZ;
DROP TABLE world_KHM;
DROP TABLE world_KIR;
DROP TABLE world_KNA;
DROP TABLE world_KOR;
DROP TABLE world_KWT;
DROP TABLE world_LAO;
DROP TABLE world_LBN;
DROP TABLE world_LBR;
DROP TABLE world_LBY;
DROP TABLE world_LCA;
DROP TABLE world_LIE;
DROP TABLE world_LKA;
DROP TABLE world_LSO;
DROP TABLE world_LTU;
DROP TABLE world_LUX;
DROP TABLE world_LVA;
DROP TABLE world_MAR;
DROP TABLE world_MCO;
DROP TABLE world_MDA;
DROP TABLE world_MDG;
DROP TABLE world_MDV;
DROP TABLE world_MEX;
DROP TABLE world_MHL;
DROP TABLE world_MKD;
DROP TABLE world_MLI;
DROP TABLE world_MLT;
DROP TABLE world_MMR;
DROP TABLE world_MNE;
DROP TABLE world_MNG;
DROP TABLE world_MNP;
DROP TABLE world_MOZ;
DROP TABLE world_MRT;
DROP TABLE world_MSR;
DROP TABLE world_MTQ;
DROP TABLE world_MUS;
DROP TABLE world_MWI;
DROP TABLE world_MYS;
DROP TABLE world_MYT;
DROP TABLE world_NAM;
DROP TABLE world_NCL;
DROP TABLE world_NER;
DROP TABLE world_NGA;
DROP TABLE world_NIC;
DROP TABLE world_NIU;
DROP TABLE world_NLD;
DROP TABLE world_NOR;
DROP TABLE world_NPL;
DROP TABLE world_NRU;
DROP TABLE world_NZL;
DROP TABLE world_OMN;
DROP TABLE world_PAK;
DROP TABLE world_PAN;
DROP TABLE world_PCN;
DROP TABLE world_PER;
DROP TABLE world_PHL;
DROP TABLE world_PLW;
DROP TABLE world_PNG;
DROP TABLE world_POL;
DROP TABLE world_PRK;
DROP TABLE world_PRT;
DROP TABLE world_PRY;
DROP TABLE world_PSE;
DROP TABLE world_PYF;
DROP TABLE world_QAT;
DROP TABLE world_REU;
DROP TABLE world_ROU;
DROP TABLE world_RUS;
DROP TABLE world_RWA;
DROP TABLE world_SAU;
DROP TABLE world_SDN;
DROP TABLE world_SEN;
DROP TABLE world_SGP;
DROP TABLE world_SHN;
DROP TABLE world_SLB;
DROP TABLE world_SLE;
DROP TABLE world_SLV;
DROP TABLE world_SMR;
DROP TABLE world_SOM;
DROP TABLE world_SRB;
DROP TABLE world_SSD;
DROP TABLE world_STP;
DROP TABLE world_SUR;
DROP TABLE world_SVK;
DROP TABLE world_SVN;
DROP TABLE world_SWE;
DROP TABLE world_SWZ;
DROP TABLE world_SYC;
DROP TABLE world_SYR;
DROP TABLE world_TCA;
DROP TABLE world_TCD;
DROP TABLE world_TGO;
DROP TABLE world_THA;
DROP TABLE world_TJK;
DROP TABLE world_TKL;
DROP TABLE world_TKM;
DROP TABLE world_TLS;
DROP TABLE world_TON;
DROP TABLE world_TTO;
DROP TABLE world_TUN;
DROP TABLE world_TUR;
DROP TABLE world_TUV;
DROP TABLE world_TWN;
DROP TABLE world_TZA;
DROP TABLE world_UGA;
DROP TABLE world_UKR;
DROP TABLE world_URY;
DROP TABLE world_USA;
DROP TABLE world_UZB;
DROP TABLE world_VAT;
DROP TABLE world_VCT;
DROP TABLE world_VEN;
DROP TABLE world_VGB;
DROP TABLE world_VIR;
DROP TABLE world_VNM;
DROP TABLE world_VUT;
DROP TABLE world_WLF;
DROP TABLE world_WSM;
DROP TABLE world_YEM;
DROP TABLE world_ZAF;
DROP TABLE world_ZMB;
DROP TABLE world_ZWE;
