#!/bin/bash
#=====================================================================
# MarlinDT Network Intelligence (MNI) - DNS Server Installer
#
...
if [[ -z "${ADDRESS}" ]] ; then
  ...
  sed -i -e "s|DNSSERV_ADDRESS=.*|DNSSERV_ADDRESS=\"${ADDRESS}\"|" ${CONFIG_DIRECTORY}/mni.ini 
  RETVAL=$?
else
...
if [[ -z "${IAM_ADDRESS}" || "${IAM_ADDRESS}" == "0.0.0.0" ]] ; then
  ...
  sed -i -e "s|IAM_ADDRESS=.*|IAM_ADDRESS=\"${IAM_ADDRESS}\"|" ${CONFIG_DIRECTORY}/mni.ini 
  RETVAL=$?
else
...
if [[ -z "${APISERV_ADDRESS}" || "${APISERV_ADDRESS}" == "0.0.0.0" ]] ; then
  ...
  sed -i -e "s|APISERV_ADDRESS=.*|APISERV_ADDRESS=\"${APISERV_ADDRESS}\"|" ${CONFIG_DIRECTORY}/mni.ini 
  RETVAL=$?
else
...
if [[ -z "${HOST}" ]] ; then
  HOST=$(hostname --short)
  sed -i -e "s|DNSSERV_HOST=.*|DNSSERV_HOST=\"${HOST}\"|" ${CONFIG_DIRECTORY}/mni.ini 
  RETVAL=$?
fi
[[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"
...
if [[ "/etc/resolv.conf" ]] ; then
 ...
 RESOLVCONF=$?
 sed -i -e "s|DNSSERV_SYSTEMD_RESOLVE=.*|DNSSERV_SYSTEMD_RESOLVE=${SYSTEMD_RESOLVE}|" ${CONFIG_DIRECTORY}/mni.ini 
 sed -i -e "s|DNSSERV_RESOLVCONF=.*|DNSSERV_RESOLVCONF=${RESOLVCONF}|" ${CONFIG_DIRECTORY}/mni.ini 
...
sed -i -e "s|group=mni|group=${GROUP}|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
...
sed -i -e "s|address=/gateway.mni.merkator.local/0.0.0.0|address=/gateway.mni.merkator.local/${APIGW_ADDRESS}|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
sed -i -e "s|srv-host=_https._tcp.gateway.mni.merkator.local,gateway.mni.merkator.local,8443,1|srv-host=_https._tcp.gateway.mni.merkator.local,gateway.mni.merkator.local,${APIGW_PORT},1|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
sed -i -e "s|address=/apiserver.mni.merkator.local/0.0.0.0|address=/apiserver.mni.merkator.local/${APISERV_ADDRESS}|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
sed -i -e "s|srv-host=_https._tcp.apiserver.mni.merkator.local,apiserver.mni.merkator.local,7443,1|srv-host=_https._tcp.apiserver.mni.merkator.local,apiserver.mni.merkator.local,${APISERV_PORT},1|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
sed -i -e "s|address=/iam.mni.merkator.local/0.0.0.0|address=/iam.mni.merkator.local/${IAM_ADDRESS}|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
sed -i -e "s|srv-host=_https._tcp.iam.mni.merkator.local,iam.mni.merkator.local,6443,1|srv-host=_https._tcp.iam.mni.merkator.local,iam.mni.merkator.local,${IAM_PORT},1|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
sed -i -e "s|mni.merkator.local|${HOST,,}.${DOMAIN,,}|g" ${WORKING_DIRECTORY}/config/dnsServer.conf && \
