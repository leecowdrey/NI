#!/bin/bash
#=====================================================================
# Network Insight (NI) - Alert Server Installer
#
...
sed -i -e "s|/usr/local/ni/fetch/fetchService.mjs|${WORKING_DIRECTORY}/fetchService.mjs|" ${WORKING_DIRECTORY}/package.json && \
...
if [[ "${CVE_SCAN,,}" == "true" ]] ; then
  ...
  CVE_TMP=$(mktemp -q -p /tmp ni.XXXXXXXX)
  cat > ${CVE_TMP} <<EOF
#!/bin/bash
