#!/bin/bash
#=====================================================================
# MarlinDT Network Intelligence (MNI) - IAM Installer (keycloak)
#
...
doing "- Copying MNI base realm"
cp -f MNI-realm.json ${WORKING_DIRECTORY}/export/ && \
cp -f MNI-users-0.json ${WORKING_DIRECTORY}/export/ && \
chown ${USERNAME}:${PASSWORD} ${WORKING_DIRECTORY}/export/MNI-realm.json && \
chown ${USERNAME}:${PASSWORD} ${WORKING_DIRECTORY}/export/MNI-users-0.json
RETVAL=$?
[[ ${RETVAL} -eq 0 ]] && success "- ok" || error "- fail"

doing "- Importing MNI base realm"
# generated previously using `bin/kc.sh export --dir=export/ --realm=MNI --optimized`
