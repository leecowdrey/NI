...
const mniMasterKey = "mniMasterKey.pem";
const mniDefaultDuration = 13;
const mniDefaultUnit = "month";
...
function help() {
  ...
  LOGGER.info("\t--verify\t\t\t\tverify encrypted key contents\n"+
     "\t\t[--in=" + mniDefaultKey + "]\t\t\tfilename to read encrypted key from\n");
...
function generate({
  ...
  let privateKeyFile = fs.readFileSync(mniMasterKey, "utf8");
...
function verify({ pemFile = mniDefaultKey } = {}) {
  let keyData = null;
  let key = null;
  let validator = new SoftwareLicenseKey(OAS.mniMasterPublicKey);
