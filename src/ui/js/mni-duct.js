//=====================================================================
// MarlinDT Network Intelligence (MNI) - JavaScript: Duct
//
...
function fetchDuctPoints() {
  ...
  if (ductId != "-1") {
    fetch(localStorage.getItem("mni.gatewayUrl") + "/duct/timeline/" + ductId, {
      method: "GET",
      headers: {
...
function fetchListDuct() {
  ...
  fetch(
    localStorage.getItem("mni.gatewayUrl") + "/duct?country=" + selectedCountry,
    {
      method: "GET",
      headers: {
        Accept: "application/json",
    ...
    .then((data) => {
      let suppliedDuctId = window.location.pathname.replace(
        localStorage.getItem("mni.rootUrl") + "/duct/",
        ""
      );
      let ductIds =
