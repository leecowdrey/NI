#!/bin/bash
#=====================================================================
# MarlinDT Network Intelligence (MNI) - UI Server Installer
#
...
if [[ -z "${ADDRESS}" ]] ; then
  ...
  sed -i -e "s|UISERV_ADDRESS=.*|UISERV_ADDRESS=\"${ADDRESS}\"|" ${CONFIG_DIRECTORY}/mni.ini 
  RETVAL=$?
else
  RETVAL=0
fi
