...
var niCountryCode = null;
var niCurrencyIsoCode = null;
var niCurrencyName = null;
var niCurrencySymbol = null;
var tickTimer = null;
...
function loadEnv() {
  try {
    ...
    niCountryCode = process.env.NI_DEFAULT_COUNTRY_CODE || "GBR";
    niCurrencyIsoCode = process.env.NI_DEFAULT_CURRENCY_ISOCODE || "GBP";
    niCurrencyName = process.env.NI_DEFAULT_CURRENCY_NANE || "Sterling";
    niCurrencySymbol = process.env.NI_DEFAULT_CURRENCY_SYMBOL || "£";
    tickIntervalMs = toInteger(process.env.UISERV_TICK_INTERVAL_MS) || 60000;
    urlPrefix = process.env.APISERV_URL_PREFIX || "/ni";
    urlVerison = process.env.APISERV_URL_VERSION || "/v1";
    //serveAddress and servePort can be overridden through DNS SD
    serveHost = process.env.DNSSERV_HOST || "NI";
    serveDomain = process.env.DNSSERV_DOMAIN || "cowdrey.local";
    serveAddress = process.env.UISERV_ADDRESS || "127.0.0.1";
    servePort = toInteger(process.env.UISERV_PORT) || 443;
    serveUrlPrefix = process.env.UISERV_URL_PREFIX || "/ni";
    serveTimeOutRequest =
    ...
    configDirectory = path.resolve(process.env.CONFIG_DIRECTORY || "/etc/ni");
    distDirectory = path.resolve(
      process.env.UISERV_DIST_DIRECTORY || "/usr/local/ni/apiServer/dist"
    );
    oauth = toBoolean(process.env.OAUTH) || false;
    oauthDiscovery = process.env.OAUTH_DISCOVERY_URL || serveUrlPrefix;
    oauthSecretId = process.env.OAUTH_CLIENT_ID || "NI";
    oauthSecret = process.env.OAUTH_CLIENT_SECRET || "NI";
...
var run = async () => {
  ...
  app.get(serveUrlPrefix + "/metadata", function (req, res, next) {
    let resJson = {
      build: appBuild,
      country: niCountryCode,
      currencyIsoCode: niCurrencyName,
      currencyName: niCurrencyName,
      currencySymbol: niCurrencySymbol,
  ...
  app.get(serveUrlPrefix + "/cable/:cableId", function (req, res, next) {
    ...
    let payload = {
      ...
      shortName: "NI",
      version: appVersion,
      country: (req.query.country ||= niCountryCode),
